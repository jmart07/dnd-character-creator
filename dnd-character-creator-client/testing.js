...
// roll four six-sided dice, sum three highest rolls (possible numbers 3 - 18)
 static generateRandomAbility {
  const sums4d4Rolls = 0;

  for(let i = 0; i < 4; i++) {
    sums4d4Rolls.push(Math.floor(Math.random() * 6) + 1);
  }

  sums4d4Rolls.sort((a, b) => b - a);

  return sums4d4Rolls[0] + sums4d4Rolls[1] + sums4d4Rolls[2];
  }
}
...
// abilities randomly generated by getAbility function, so possible return values is array with six numbers each from 3 - 18 inclusive
 generateRandomAbilities = () => {
  ...
  for(let i = 0; i < 6; i++) {
    randomAbilities.push(generatesRandomAbility());
  }
